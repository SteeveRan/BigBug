 ...
 * @description Страница списка Source Repositories — таблица с фильтрами (Group F)
...
const RepositoriesPage = () => {
  ...
  // Fetch repositories for selected group
  const {
    data: repositories = [],
  ...
  return (
    <Flex vertical gap={16}>
      ...
      {!isLoading && !reposError && effectiveGroupId == null && groups.length === 0 && (
        <Card>
          <Empty description="No groups available. Import a group first." />
        </Card>
      )}

      {!isLoading && !reposError && effectiveGroupId != null && (
        <Card>
          <Table
            columns={columns}
