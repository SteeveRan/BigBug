...
export function MirrorDetailPage() {
  ...
  const handleSaveCron = async () => {
    await updateSchedule({ id: mirrorId, data: { cron_expression: cronExpr, use_default_schedule: false } })
  }
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
        <Button startIcon={<ArrowBack />} onClick={() => navigate('/mirrors')}>Back</Button>
      ...
      <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
        <Card sx={{ flex: '1 1 300px' }}>
          <CardContent>
            <Typography variant="h6" mb={2}>Mirror Info</Typography>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
              <Box>
                <Typography variant="caption" color="text.secondary">Status</Typography>
                <Box mt={0.5}>
                  <StatusChip statusFlag={m.status_flag as 0|1|2|3|4} statusText={m.status_text} />
                </Box>
              </Box>
              <Divider />
              <Box>
              ...
              <Divider />
              <Box>
                <Typography variant="caption" color="text.secondary">Last Synced Release</Typography>
        ...
        {s && (
          <Card sx={{ flex: '1 1 300px' }}>
            <CardContent>
              ...
              />
              <FormControlLabel
                control={<Switch checked={s.use_default_schedule} onChange={handleToggleDefault} disabled={!s.is_enabled} />}
